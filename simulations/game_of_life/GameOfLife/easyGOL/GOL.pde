...
class GOL {
  ...
  private int getPerc(float perc) { return round(perc * 100); }
  
  public int getCellsAlive() { return getCellsAlive(false); }
  public int getCellsAlive(boolean percentage) {
    if (percentage) { return getPerc(cells_alive / (float) getCellsTotal()); }
    return cells_alive;
  }
  
  public int getCellsDead() { return getCellsDead(false); }
  public int getCellsDead(boolean percentage) {
    int cells_dead = getCellsTotal() - getCellsAlive(); 
    if (percentage) { return getPerc(cells_dead / (float) getCellsTotal()); }
    return cells_dead;
  }
  ...
  public int step() {
    ...
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        ...
        boolean cell_alive = applyRules(grid[y][x], n_active);
        neighbours[y][x] = cell_alive;
        if (cell_alive) { c_alive++; }
        if (cell_alive != grid[y][x]) { changed++; }
